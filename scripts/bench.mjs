...
//
// That makes it a load generator with forged client addresses, so it only ever
// talks to an API on this machine: the base URL must be http on a loopback host.
import { performance } from "node:perf_hooks"

const LOOPBACK_HOSTS = ["localhost", "127.0.0.1", "[::1]"]

// Rebuilt from a fixed host and a numeric port rather than passed through, so
// nothing else from the command line reaches a request.
const loopbackBase = (argument) => {
  let url
  try {
...
const base = loopbackBase(process.argv[3] ?? "")

if (!label || !base) {
  console.error(
    "Usage: node bench.mjs <label> <base url>\nThe base URL must be an http address on this machine, e.g. http://localhost:3000",
  )
  process.exit(1)
}
...
const books = await (
  await fetch(`${base}/v1/books`, { headers: headers() })
).json()
const chapters = []
...
for (const path of [0, 1, 2, 3, 4, 5, 1000, 1100, 1200].map(
  ...
  for (const v of chapter.verses)
    for (const t of v.text) {
      for (const w of (t.normalizedText ?? "").match(/[a-z]{4,}/g) ?? [])
        words.add(w)
    }
}
...
for (const r of rows) {
  console.log(
    `${r.name.padEnd(22)} ${String(r.c).padStart(4)} ${String(r.n).padStart(5)} ${f(r.rps, 0).padStart(7)} ${f(r.p50, 1).padStart(6)}ms ${f(r.p95, 1).padStart(6)}ms ${f(r.p99, 1).padStart(6)}ms  ${r.errors}`,
  )
}
